...
def _find_cmd(program, env=None, cwd=None):
    import os
    def is_exe(fpath):
    ...
    if fpath:
        ...
        if cwd: environ["PATH"] += ":%s" % os.path.abspath(cwd)
...
class CommandNotFound(Exception):
    def __init__( self, command ):
        ...
        Exception.__init__(self, 'Command not found: missing %s' % command)
...
def run(command, data=None, timeout=None, kill_timeout=None, env=None, cwd=None):
    ...
    # try:
    #     for cmd in command:
    #         if not _find_cmd(cmd[0], env, cwd):
